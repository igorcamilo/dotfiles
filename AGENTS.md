...
machine, terminology, file layout, installation flow, and security model.
...
- Give each module one clear owner, such as storage or boot.
...
- Obtain temporary tools from the locked `nixpkgs` input with
  ...
  flake.
- `check-system.sh` evaluates and builds the host on its native architecture.
...
- Do not add emulation or cross-compilation for this machine.
...
- `flake.nix`: locked inputs, shared module composition, the NixOS output, and
